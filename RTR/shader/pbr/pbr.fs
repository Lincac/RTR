...
float ShadowCalculation(vec3 fragPosWorldSpace){
    ...
    vec3 normal = normalize(texture(Normal,fs_in.TexCoords).xyz);
    float bias = max(0.05 * (1.0 - dot(normal,normalize(lightPos))),0.005);
...
void main(){
    ...
    vec3 L = normalize(lightPos - fs_in.FragPos); // 直接光
